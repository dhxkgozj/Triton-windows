...
uint64 AnalysisProcessor::getSSERegisterValue(uint64 regID)
{
	if (!this->currentCtxH)
		return 0;
...
void AnalysisProcessor::setSSERegisterValue(uint64 regID, uint64 value)
{
	if (!this->currentCtxH)
		return;
...
uint64 AnalysisProcessor::getMemValue(uint64 mem, uint32 readSize)
{
	return this->currentCtxH->getMemValue(mem, readSize);
}


void AnalysisProcessor::setMemValue(uint64 mem, uint32 writeSize, uint64 value)
{
