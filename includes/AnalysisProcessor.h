...
public:
	...
	uint64     getSSERegisterValue(uint64 regID);

	/* Set the value into the register */
	void        setRegisterValue(uint64 regID, uint64 value);
	void        setSSERegisterValue(uint64 regID, uint64 value);

	/* Returns the value of the memory */
	uint64     getMemValue(uint64 mem, uint32 readSize);
	void        setMemValue(uint64 mem, uint32 writeSize, uint64 value);
	
	/*
	 *Taint Engine part
	 *--------------------
	 */
	/*return the taint engine reference*/
