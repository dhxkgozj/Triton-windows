...
// There is no verification on the validity of the ID.
uint64 PINContextHandler::getSSERegisterValue(uint64 TritRegID) const
{
  REG reg                 = safecast(PINConverter::convertTritonReg2DBIReg(TritRegID));
  uint64 value       = 0;
  PIN_REGISTER tmp;
  ...
  value = *reinterpret_cast<uint64*>(&tmp);
...
// There is no verification on the validity of the ID.
void PINContextHandler::setSSERegisterValue(uint64 TritRegID, uint64 value) const
{
  REG reg = safecast(PINConverter::convertTritonReg2DBIReg(TritRegID));
...
uint64 PINContextHandler::getMemValue(uint64 mem, uint32 readSize) const
{
  ...
  switch(readSize){
    case BYTE_SIZE:   return static_cast<uint64>(*(reinterpret_cast<uint8 *>(mem)));
    case WORD_SIZE:   return static_cast<uint64>(*(reinterpret_cast<UINT16 *>(mem)));
    case DWORD_SIZE:  return static_cast<uint64>(*(reinterpret_cast<uint32 *>(mem)));
    case QWORD_SIZE:  return static_cast<uint64>(*(reinterpret_cast<uint64 *>(mem)));
  }
...
void PINContextHandler::setMemValue(uint64 mem, uint32 writeSize, uint64 value) const
{
